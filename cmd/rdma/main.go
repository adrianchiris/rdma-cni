...
import (
	...
	"github.com/adrianchiris/rdma-cni/pkg/cache"
	"github.com/adrianchiris/rdma-cni/pkg/rdma"
	rdmatypes "github.com/adrianchiris/rdma-cni/pkg/types"
	"github.com/adrianchiris/rdma-cni/pkg/utils"
...
func (plugin *rdmaCniPlugin) ensureRdmaSystemMode() error {
	...
	if mode != rdma.RdmaSysModeExlusive {
		return fmt.Errorf("RDMA subsystem namespace awareness mode is set to %s, "+
			"expecting it to be set to %s, invalid system configurations", mode, rdma.RdmaSysModeExlusive)
	}
	return nil
}
...
func (plugin *rdmaCniPlugin) moveRdmaDevToNs(rdmaDev string, nsPath string) error {
	...
	targetNs, err := plugin.nsm.GetNS(nsPath)
	if err != nil {
...
func (plugin *rdmaCniPlugin) moveRdmaDevFromNs(rdmaDev string, nsPath string) error {
	...
	sourceNs, err := plugin.nsm.GetNS(nsPath)
	if err != nil {
		return fmt.Errorf("failed to open network namespace %s: %v", nsPath, err)
	}
	defer sourceNs.Close()

	targetNs, err := plugin.nsm.GetCurrentNS()
	if err != nil {
...
func (plugin *rdmaCniPlugin) CmdAdd(args *skel.CmdArgs) error {
	...
	// Move RDMA devices to container namespace
	rdmaDev := rdmaDevs[0]
	...
	if err != nil {
		...
		if restoreErr != nil {
			return fmt.Errorf("failed while restoring namespace for RDMA device %s. %v", rdmaDev, err)
		}
		return err
	}
